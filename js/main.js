...
function ThemSinhVien() {
    ...
    if (KiemTraDauVaoRong("Toan", Toan) == true) {
        loi++;
    }
    if (KiemTraDauVaoRong("Ly", Ly) == true) {
        loi++;
    }
    if (KiemTraDauVaoRong("Hoa", Hoa) == true) {
        loi++;
    }
    if (loi != 0) {
        return;
    ...
    //Thêm sinh viên 
    var sinhvien = new SinhVien(masv, hoten, email, cmnd, sdt);
...
function CapNhatDanhSachSV(DanhSachSinhVien) {
    ...
    for (var i = 0; i < DanhSachSinhVien.DSSV.length; i++) {
        ...
        //Tạo thẻ td và filter dữ liệu sinh viên thứ [i]vào
...
function GetStorage() {
    //Lấy ra chuỗi json là mảng danhsachsinhviên thông qua tên DanhSachSV
